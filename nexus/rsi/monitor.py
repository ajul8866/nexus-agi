...
class PerformanceMonitor:
    ...
    def record_task(self, task_id: str, success: bool, latency: float,
        ...
        record = TaskRecord(
            task_id=task_id, success=success, latency=latency,
            tokens_used=tokens_used, quality_score=quality_score,
    ...
    def _check_alerts(self, record: TaskRecord) -> None:
        ...
        for metric, threshold in self.THRESHOLDS.items():
            ...
            if metric == "avg_latency" and val > threshold:
                self._alerts.append({"metric": metric, "value": val, "threshold": threshold, "timestamp": time.time()})
        ...
        for metric, threshold in self.THRESHOLDS.items():
            ...
            if is_bad:
                ...
                bottlenecks.append(BottleneckReport(
                    metric=metric, severity=severity, current_value=val,
                    threshold=threshold, suggestion=suggestions.get(metric, "Investigate further"),
                    priority=priority
                ))
                priority += 1
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            "summary": metrics,
            "bottlenecks": [{"metric": b.metric, "severity": b.severity, "suggestion": b.suggestion}
            ...
            "trends": {
                ...
                "recent_success_rate": round(sum(1 for r in records[-10:] if r.success) / max(len(records[-10:]), 1), 3)
            },
